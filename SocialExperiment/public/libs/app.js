...
window.harmonize = function(newNote, falloff, duet) {
	duet = duet === true
	var jump = 1;
	if( duet ) jump = 2

  for (var i=0; i<filterNodes.length; i+=jump){
...
function createSparkle(x, y, color){
	for (var i=0; i<5; i++) {
    newParticle = new Particle(x + Math.random()*40, y + Math.random()*40, + Math.random()*20, color);
    particles.push(newParticle);
  }  
}

function createSound(x, y, duet){
	var mousePos    = (x/v_ctx_width);
  var curNote     = lowestNote + Math.round(mousePos * noteRange);
  window.harmonize(curNote, (y/v_ctx_height)*4, duet);
  for (var i=0; i<5; i++) {
    newParticle = new Particle(x + Math.random()*40, y + Math.random()*40, + Math.random()*20);
    particles.push(newParticle);
  }  	
}
...
var handleOtherPlayer = function(otherPlayer, index, otherPlayerOld){
	var randomColor = '#'+Math.floor(Math.random()*16777215).toString(16);

	if( !_.isEqual(otherPlayer.mouse, otherPlayerOld.mouse) )
		createSparkle( otherPlayer.mouse.x, otherPlayer.mouse.y, randomColor )
	
	if( !_.isEqual(otherPlayer.click, otherPlayerOld.click) )
		createSound( otherPlayer.click.x, otherPlayer.click.y, true )
	
}	

// Setup observers of other people
setTimeout(function(){
	var recency = 10 * 1000
	var query = Players.find({
		lastUpdate: {$gt: (new Date()).getTime()-recency},
		_id: { $ne: playerId }
	})

	var handle = query.observe({
		changed: handleOtherPlayer,
	})
}, 100)
